...
| **A scheme is inverted by re-solving it** — the masses carrying type over photography keep their original register, the one tonal exception inverts the other way, and a light ladder holds fewer steps than the dark one it came from | [color](skills/color-taste/SKILL.md#invariant) |
...
| **Motion is bound to roles, not to instances** — a system written against one page's own sections silently applies to nothing on the next page, and a silent no-op passes every check that asks whether something is broken | [motion-judgment](skills/motion-judgment/SKILL.md#invariant) |
