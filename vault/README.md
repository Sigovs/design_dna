...
| 2026-08-05 | all 22 records outside Semler and Rimac | **Searched for `technical-luxury`'s third observation. Not found — provisional stands.** Read against the chain Alex set, *technical fact → observable consequence → demonstrated value*, rather than against technical appearance. Oil Stain Lab presents specification as drama; Hispano Suiza builds mythology; McLaren makes cars *feel* powerful by scale; augen.pro is immersive rather than evidential; HB Body carries the closest logic and is `out`, so it cannot carry a dialect at all. Nearest miss recorded in the dialect file: `porsche-com-usa`, whose record already describes specification as classified content — but it is `hybrid`, and the test counts `in`. **A negative result is a result**, and it is logged so the next pass does not re-read the same records hopefully. |
| 2026-08-05 | the four `out` records · the three `NO RULE COVERS THIS` groups | **One promotion, three holds, nothing merged.** `technical-luxury` re-declared **provisional** by Alex — two independent observations (Semler Premium as one, Rimac Nevera as two), one short. The four `out` records were **not** merged into a dialect: RM Sotheby's is an anti-reference, The Gentlewoman a single strong device with a recorded contradiction, HB Body spatial industrial-editorial, Rivian human-centred adventure technology — no shared governing logic, and grouping them would classify by the `out` label rather than by a decision. The three uncovered groups are held as **candidate tensions** in [EVIDENCE.md §E](EVIDENCE.md), not promoted: two of them rest on one record. |
...
| 2026-08-13 | the 19 records added since 2026-07-30 — the first full run | **Four amendments, all approved by Alex, none of them a new rule invented from one record.** `academic-composition` **C19** added as an INVARIANT — *the page holds its standard to the last mass* — on ten records spanning `in` and `out`, one rated 3, with the full-page capture of `polestar-com-us` as the visual proof: one authored typographic event, then a disclaimer wall and a link farm. Alex set the tier and gave the reason: a utilitarian page may be calm, dense and conventional and still comply, because the rule forbids formlessness rather than restraint. **C17 widened** from a class of asset to a class of asset *or condition*, and — his correction — the test is no longer "the composition must hold when the condition slips" but the plausible variation the system is expected to survive: `eco-com` is an authored constraint, `mugenstudio-framer-website` an operational risk, because a template advertises reuse. **U12 gained the symptom it was missing** — routes that are visible, labelled and still indistinguishable, with `mugenstudio` as the positive case and its own voiding condition. **MJ2** now names continuity between chapters as structural work; Alex rejected the absolute boundary and replaced it: repetition alone does not remove a transition from band 2, only a device applied regardless of what the seam needs to express does. Nothing promoted from `EVIDENCE.md §E` — see the note below. |
| 2026-08-13 | `EVIDENCE.md` §E2, reopened on its own | **Promoted — the candidate is closed.** It became `motion-judgment` **MJ10**, *a device earns its advance by what it delivers*, at INVARIANT tier. Reading every record moved the count from three to five, three of them rating 3: `oilstainlab-com` and `hispanosuizacars-com` had been carrying the theme in prose and were never tagged for it. The wording written from `trionn-com` alone did not survive — all five records say something narrower and checkable, and the removal test is what makes the rule usable. MJ6 handed over its scroll-budget clause and kept agency as its subject. |
| 2026-08-13 | seven Japanese design principles, submitted by Alex as an external article | **Nothing promoted, and no dialect created.** `refined-elegance` and `organic-tactile` already carry roughly 70% of the logic — interval over volume, hairlines over containers, authored deviation from an axis — so a tenth dialect would have been vocabulary inflation rather than a new way of deciding. Alex's instruction stands with it: **the system keeps its own lexicon and does not become a catalogue of cultural terms**, so none of these are named in Japanese in any rule. Three items **carried to the next run — external hypotheses, no supporting vault evidence**: · **Intentional withholding** (*yūgen*) — **do not promote.** [C18](../skills/academic-composition/SKILL.md#invariant) already licenses the permitted case, where the Read declares the withholding as the subject, and the vault evidence currently runs the other way: every judged instance is a criticism — `hispanosuizacars-com` *"mystery should create curiosity, but the site occasionally assumes the visitor already understands"*, `oilstainlab-com` *"creates mystery, but this is still a highly specialized product requiring trust"*, `semlerpremium-dk-showroom` *"withholds information comparison shoppers may want immediately"*. · **The second look** (the half of *shibui* the system has no term for) — **watch list.** A search of all 31 records returned zero sentences about repeated viewing, returning to a page or a second read; the system knows only the negative form, in the critique panel's *"what will annoy on the 50th use"*. The boundary if it is ever written: the first look may never be spent to buy the second. · ***Shizen*** — held as an **external contradiction with [U8](../skills/anti-patterns/SKILL.md#invariant)**, not as a rule and not as evidence of Alex's taste: it asks a composition to look grown and unforced, U8 requires irregularity to read as intentional. One axis, opposite ends. `rekorderstudios-com` (3, in) sits on it — *"some compositions approach accidental imbalance … can appear like rendering errors"* — but that is a record's weakness, not support for the theory. **None of the three entered `EVIDENCE.md`, `refined-elegance`, `organic-tactile`, an invariant or a dialect.** |
